...
bool flags_get(flags_OpenFlags p_base, flags_Flag p_flag) {
    return (int32_t)((uint32_t)p_base.m_value & (uint32_t)p_flag.m_value) == p_flag.m_value;
}

void flags_set(flags_OpenFlags* p_base, flags_Flag p_flag, bool p_set) {
    p_base->m_value = (int32_t)((uint32_t)p_base->m_value | (uint32_t)(p_flag.m_value * p_set));
}
...
void flags_open(flags_String p_fn, flags_OpenFlags p_flags) {
    if (flags_get(p_flags, (flags_Flag){O_RDWR})) {
    } else {
        flags_OpenFlags tmp_0 = (flags_OpenFlags){0};
        flags_set(&tmp_0, (flags_Flag){O_RDONLY}, true);
        flags_set(&tmp_0, (flags_Flag){O_APPEND}, true);
        if (p_flags.m_value == tmp_0.m_value) {
        } else if (flags_get(p_flags, (flags_Flag){O_RDONLY})) {
        } else if (flags_get(p_flags, (flags_Flag){O_WRONLY})) {
        }
    }
}
...
void flags_test1(void) {
    ...
    flags_set(&tmp_1, (flags_Flag){O_RDONLY}, true);
    flags_open(tmp_0_arg, tmp_1);
    flags_String tmp_3_arg = flags_string((int8_t*)"file.txt", 8);
    flags_OpenFlags tmp_2 = (flags_OpenFlags){0};
    flags_set(&tmp_2, (flags_Flag){O_RDONLY}, true);
    flags_open(tmp_3_arg, tmp_2);
}
...
void flags_test2(void) {
    ...
    flags_set(&tmp_0, (flags_Flag){O_RDONLY}, true);
    flags_OpenFlags tmp_1 = (flags_OpenFlags){0};
    flags_set(&tmp_1, (flags_Flag){O_WRONLY}, true);
    flags_open(tmp_2_arg, (flags_OpenFlags){(int32_t)((uint32_t)tmp_0.m_value | (uint32_t)tmp_1.m_value)});
    flags_String tmp_4_arg = flags_string((int8_t*)"file.txt", 8);
    flags_OpenFlags tmp_3 = (flags_OpenFlags){0};
    flags_set(&tmp_3, (flags_Flag){O_WRONLY}, true);
    flags_set(&tmp_3, (flags_Flag){O_APPEND}, true);
    flags_open(tmp_4_arg, tmp_3);
}

void flags_test3(void) {
    flags_OpenFlags l_flags = {0};
    flags_set(&l_flags, (flags_Flag){O_WRONLY}, true);
    flags_String tmp_0_arg = flags_string((int8_t*)"file.txt", 8);
    flags_open(tmp_0_arg, l_flags);
    flags_String tmp_2_arg = flags_string((int8_t*)"file.txt", 8);
    flags_OpenFlags tmp_1 = l_flags;
    flags_set(&tmp_1, (flags_Flag){O_APPEND}, true);
    ...
    flags_set(&tmp_5, (flags_Flag){O_RDONLY}, true);
