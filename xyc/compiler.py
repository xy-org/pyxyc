...
def compile_vardecl(node, cast, cfunc, ctx):
    ...
    if type_desc.is_ct_only:
        raise CompilationError("Cannot create variable with compile-time only type", node)
...
def do_compile_expr(expr, cast, cfunc, ctx: CompilerContext, deref=True, allow_poison=False) -> ExprObj:
    ...
    elif isinstance(expr, xy.FuncType):
        type_obj = find_type(expr, cast, ctx)
        return ExprObj(
            xy_node=expr,
            c_node=c.Id("NULL"),
...
def compile_builtin_fcall(expr, func_obj, arg_exprs: ArgList, cast, cfunc, ctx):
    ...
    if is_builtin_func(func_obj, "get"):
        ...
        return compile_builtin_to(arg_exprs[0], arg_exprs[1].inferred_type, expr)
    elif is_builtin_func(func_obj, "toBits"):
        natural_bit_type = ctx.prim_bit_objs[2]
        if arg_exprs[0].inferred_type.name in {"Byte", "Ubyte"}:
            natural_bit_type = ctx.prim_bit_objs[0]
        elif arg_exprs[0].inferred_type.name in {"Short", "Ushort"}:
            natural_bit_type = ctx.prim_bit_objs[1]
        elif arg_exprs[0].inferred_type.name in {"Long", "Ulong", "Double"}:
            natural_bit_type = ctx.prim_bit_objs[3]
        return compile_builtin_to(arg_exprs[0], natural_bit_type, expr)
        ...
        gen_args = ArgList()
        for p in params:
            ctx.ns[p.xy_node.name] = p
            gen_args.args.append(ExprObj(
                c_node=c.Id(p.c_node.name),
                xy_node=p.xy_node,
                inferred_type=p.type_desc,
            ))
        fcall_obj = do_compile_fcall(expr, func_obj, gen_args, cast, gen_cfun, ctx)
        ctx.pop_ns()

        if fcall_obj.inferred_type.c_name == "void":
            gen_cfun.body.append(fcall_obj.c_node)
        else:
...
def compile_builtin_to(from_obj: CompiledObj, dst_type: CompiledObj, expr):
    src_type = from_obj.inferred_type
    ...
    if is_fp_to_bits_or_bits_to_fp:
        if isinstance(from_obj.c_node, c.Id):
            # produce cleaner code when first arg is a variable name
            c_res = c.UnaryExpr(from_obj.c_node, op="&", prefix=True)
            c_res = c.Cast(c_res, to=f"{dst_type.c_node.name}*")
    ...
    what = from_obj.c_node
    to = dst_type.c_node.name
...
def compile_list_comprehension(expr: xy.ListComprehension, cast, cfunc, ctx: CompilerContext, is_assign=False):
    ...
    if isinstance(container_obj.inferred_type, ArrTypeObj):
        ...
        for func_type_obj in func_type_objs:
            ctx.ns[iter_var_name.name] = func_type_obj
            elem_obj = compile_expr(expr.loop.block.body, cast, cfunc, ctx)
            if dst_obj is None:
                c_node.elems.append(elem_obj.c_node)
            else:
...
def mangle_fptr(func_obj: FuncObj, ctx):
    param_objs = func_obj.param_objs
    rtype_obj = func_obj.rtype_obj
    mangled = ["xy_fp"]
    for param_obj in param_objs:
        if param_obj.xy_node.mutable:
            mangled.append("__mut")
        mangled.append("__")
        do_mangle_type_obj(mangled, param_obj.type_desc)
    mangled.append("__")
    if func_obj.etype_obj is not None:
        mangled.append("mut__")
    do_mangle_type_obj(mangled, rtype_obj)
    if func_obj.etype_obj is not None:
        mangled.append("__")
        do_mangle_type_obj(mangled, func_obj.etype_obj)

    mangled = "".join(mangled)
    return mangled

def create_fptr_type(func_obj: FuncObj, cast, ctx: CompilerContext):
    param_objs = func_obj.param_objs
    rtype_obj = func_obj.rtype_obj
    c_typename = mangle_fptr(func_obj, ctx)
    if c_typename not in ctx.defined_c_symbols:
        ctx.defined_c_symbols.add(c_typename)
        param_types = ', '.join(p.c_node.qtype.type.name for p in param_objs)
        if func_obj.etype_obj is not None:
            param_types = ', '.join((param_types, func_obj.rtype_obj.c_name + "*"))
        if len(param_types) == 0:
            param_types = "void"
        rtype_cname = rtype_obj.c_name if func_obj.etype_obj is None else func_obj.etype_obj.c_name
        c_typedef = c.Typedef(
            f"{rtype_cname} (*{c_typename})({param_types})",
            "",
